...
In the file [`Pipfile`](https://github.com/Release-Candidate/Tzolkin/blob/main/Pipfile) there is a stanza saying
...
The Markdown documentation for GitHub are the files [README.md](https://github.com/Release-Candidate/tzolkin-calendar/blob/main/README.md) and [CHANGELOG.md](https://github.com/Release-Candidate/tzolkin-calendar/blob/main/CHANGELOG.md) in the project root directory.
...
configured in the file [`.config/dotnet-tools.json`](https://github.com/Release-Candidate/Tzolkin/blob/main/.config/dotnet-tools.json)
...
- All build artifacts are generated in the directory `artifacts`, set using the expression
  `buildOutputPath` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx).
- NuGet packages generated by the target `Packages` are saved to the directory `packages`, set using the expression
  `packageOutputPath` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx).
- The test results of the targets `Tests` and `TestsDeb` are saved as a `.trx` file in the
  directory `test_results`, set using the expression  `testOutputPath` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx).
...
- `Build` and `BuildDeb` build the configured projects
   ...
   `src` and `tests`. The expression `buildProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx) defines
   the projects to build.
- `Docs` generates the HTML documentation using MkDocs. The HTML is created in the directory `sites`.
  The output directory is configured in the MkDocs config file [`mkdocs.yml`](https://github.com/Release-Candidate/Tzolkin/blob/main/mkdocs.yml)
- `Lint` runs FSharpLint on all configured projects. Default is all projects, `lintProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx)
...
- `Tests` and `TestsDeb` build and run all test projects in the directory `tests` using `dotnet test`.
  ...
  Default is testing all projects in the directory `tests`, `testProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx) defines the projects to run `Tests` and `TestsDeb` on.
- `TestsCoverage` and `TestsCoverageDeb`  build and run all tests projects and generate a coverage report using AltCover.
  ...
  Default is testing all projects in the directory `tests`, `testProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx) defines the projects to run `TestsCoverage` and `TestsCoverageDeb` on.
- `Publish` runs `dotnet publish` on the configured projects, default is all in `src`. The
  ...
  and be the same as the newest one in [CHANGELOG.md](https://github.com/Release-Candidate/Tzolkin/blob/main/CHANGELOG.md).
  `publishProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx) defines the projects to run `Publish` on.
...
- `Packages` builds the NuGet packages of all projects using a release build. The
  build is done with the release configuration, the version **must** be given as argument on the command line
  and be the same as the newest one in [CHANGELOG.md](https://github.com/Release-Candidate/Tzolkin/blob/main/CHANGELOG.md).
  `packageProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx) defines the projects to run `Packages` on.
...
- `VERSION_STRING` is the version to use for the target, **must** be the same as the first (newest)
  version in the file [`CHANGELOG.md`](https://github.com/Release-Candidate/Tzolkin/blob/main/CHANGELOG.md). Targets that need `VERSION_STRING` are
...
Run `dotnet publish` on the configured projects (`publishProjs` in [`build.fsx`](https://github.com/Release-Candidate/Tzolkin/blob/main/build.fsx))  setting the
version to 6.2.53
...
- `create_packages.yml` creates and uploads the NuGet packages, runs `dotnet publish` and
  ...
  the source with a release tag of the form `v?.?.?` (**must be the same version as the newest in [CHANGELOG.md](https://github.com/Release-Candidate/Tzolkin/blob/main/CHANGELOG.md)**).
  Appends the newest entry in [CHANGELOG.md](https://github.com/Release-Candidate/Tzolkin/blob/main/CHANGELOG.md) to the release - script [`scripts/get_changelog.sh`](https://github.com/Release-Candidate/Tzolkin/blob/main/scripts/get_changelog.sh)
  See the [latest release](https://github.com/Release-Candidate/Tzolkin/releases/latest) as an example
...
The badges of the workflows are linked in the section [Badges](https://github.com/Release-Candidate/Tzolkin#badges )
