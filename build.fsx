...
let builOutAbs = Path.combine rootDirAbs buildOutputPath // build output dir, absolute
...
let commonBuildOpts =
    { commonDotNetOpts with
          CustomParams =
              Some((sprintf "/p:BaseIntermediateOutputPath=%s/ /p:IntermediateOutputPath=%s/" builOutAbs builOutAbs)) }
          ...
          Common =
              { commonDotNetOpts with
                    CustomParams =
                        Some(
                            sprintf
                                "/p:OutputPath=%s /p:BaseIntermediateOutputPath=%s/ /p:IntermediateOutputPath=%s/ /property:Version=%s  -p:FileVersion=%s"
                                builOutAbs
                                builOutAbs
                                builOutAbs
                                version
...
Target.create
    ...
    (fun p ->
        ...
        publishProjs
        |> Seq.iter (DotNet.publish (setPublishOptions (getRID ()) (getVersion changelogVers argVersion))))
